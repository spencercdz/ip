...
Aim: Verify the GUI presents Jaku as a calm task companion, sends a command with both Enter and Send, displays asymmetric user and Jaku dialogs in order without console divider lines, highlights errors, adapts to minimum and expanded window widths, scrolls to new dialogs, restores command focus, and closes after `bye`.
